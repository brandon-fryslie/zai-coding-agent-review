...
How tight the check is depends on the credential, and the weaker case is the default one. A user PAT in `GITHUB_REVIEW_TOKEN` names its own account, so the action matches that account exactly and no one else can forge a round. The default `GITHUB_TOKEN` cannot name itself — GitHub gives an installation token no way to learn its own app — so there the action can only match *a bot*. The bar on that path is "a privileged app already installed on this repo" — Dependabot, Renovate, a sibling workflow — rather than "this action". That is a large reduction from *any account with read access*, and not an elimination.

**On Gitea this gate is not yet measured — set `GITHUB_REVIEW_TOKEN` to a user PAT.** The action resolves its identity the same way on both hosts, but only the GitHub behaviour was confirmed against a live Actions run. The Gitea half rests on the expectation that a runner's own job token is served by `GET /user`, which is reasonable and unverified. That matters more than an ordinary unknown would, because an identity the action cannot establish fails the run rather than guessing: if `GET /user` refuses an `act_runner` job token for any reason, the fallback probe is a GitHub-Apps endpoint Gitea has no reason to implement, and every run on that host fails rather than merely losing the check. A user PAT sidesteps the question — a PAT names its own account, and Gitea serves `/user` for one (measured against Gitea 1.27.2) — so on Gitea, configure `GITHUB_REVIEW_TOKEN` and the identity resolves through the path known to work.
