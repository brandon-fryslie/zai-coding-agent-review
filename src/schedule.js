...
//
// The recorded schedule value (minted at the one spawn seam in src/multiscope.js):
...
function describeSchedule({ scopeConcurrency, sweepCap, scopeCount, spawns }) {
  const scoutMs = sumMs(spawns.filter(s => s.phase === 'scout').map(s => spanMs(s.usage && s.usage.span)));
  const byPass = new Map();
  for (const s of spawns) {
    if (s.phase !== 'worker') continue;
...
module.exports = { spanMs, sumMs, describeSchedule };
