...
**Once per cause applies to the round cap, not to fork skips.** A fork PR gets a notice on every push, deliberately. The "already told them" check reads the markers in the PR's existing reviews, and on a fork PR anyone with read access can post a review — including its author, who could plant the marker and switch off the warning about their own unreviewed PR. A PR the action refuses to review because it does not trust it cannot also be a PR whose reviews it trusts. A same-repo PR's reviews come only from accounts with push access, so the round cap keeps its check.
