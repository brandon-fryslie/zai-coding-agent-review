...
**Every marker the action reads is checked against its author first.** On a public repo anyone with read access can submit a review, so a body ending in the action's marker could once forge a review *round* — and enough forged rounds push a PR past its cap so it is never reviewed again. The action now resolves the identity it posts under (once per run, from the token that actually posts) and ignores any review written by anyone else, before the body is examined. That single check covers everything derived from the PR's history: the round count, the cost totals, which inline comments count as the action's own earlier findings, which blocking review it may dismiss, and the notice's own de-duplication. If that identity cannot be established, the run fails loudly rather than guessing — guessing one way would trust a stranger, and the other would uncap review spend.
