...
**Every marker the action reads is checked against its author first.** On a public repo anyone with read access can submit a review, so a body ending in the action's marker could once forge a review *round* — and enough forged rounds push a PR past its cap so it is never reviewed again. The action now resolves the identities it posts under, once per run, and ignores any review written by anyone else before the body is examined. That single check covers everything derived from the PR's history: the round count, the cost totals, which inline comments count as the action's own earlier findings, which blocking review it may dismiss, and the notice's own de-duplication. If an identity cannot be established, the run fails loudly rather than guessing — guessing one way would trust a stranger, and the other would uncap review spend.

It resolves *every* token the run holds, not just the one that posts, so **adding `GITHUB_REVIEW_TOKEN` to a repo with pull requests already in flight is safe**: rounds posted earlier under the default token stay recognized, keeping the round count intact and leaving an outstanding change request releasable. Rotating an existing `GITHUB_REVIEW_TOKEN` to a *different* user account is the one transition this cannot cover — nothing can tell the action about an account it no longer holds a token for — so on such a PR the earlier rounds stop counting and a blocking review from before the rotation has to be dismissed by hand.
